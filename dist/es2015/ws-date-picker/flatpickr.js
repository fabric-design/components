...
function Flatpickr(element, config) {
	...
	function updateTime(e) {
		if (self.config.noCalendar && !self.selectedDates.length) self.selectedDates = [self.now];
	...
	function clear(triggerChangeEvent) {
		...
		if (triggerChangeEvent !== false) triggerEvent("Change");
	}
...
Flatpickr.prototype = {
	...
	parseDate: function parseDate(date, timeless, givenFormat) {
		...
		if (date.toFixed) date = new Date(date);else if (typeof date === "string") {
			...
			if (date === "today") {
				date = new Date();
				timeless = true;
			} else if (this.config && this.config.parseDate) date = this.config.parseDate(date);else if (/Z$/.test(date) || /GMT$/.test(date)) date = new Date(date);else {
		...
		} else if (date instanceof Date) date = new Date(date.getTime());
