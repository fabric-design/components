...
In some use cases it might be the best to only show the dropdown items, when the user entered a valid filter value.
With the option `minFilterLength` you can specify how many characters minimum has to be entered to show items.
If the minimum length is not fulfilled no item will be displayed but a hint that the filter has to be used.

<ws-dropdown items.bind='["Car (Blue)", "Car (Red)", "Car (Orange)", "Bus (Blue)", "Bus (Red)", "Bus (Orange)", "Jet (Blue)", "Jet (Red)", "Jet (Orange)"]' filterable="1" min-filter-length.bind="2" limit="7" text="Min length: 2" type="select" style="display: inline-block; width: 250px"></ws-dropdown>
```html
<ws-dropdown
  items.bind='["Car (Blue)", "Car (Red)", "Car (Orange)", "Bus (Blue)", "Bus (Red)", "Bus (Orange)", "Jet (Blue)", "Jet (Red)", "Jet (Orange)"]'
  filterable="1"
  min-filter-length.bind="3"
  limit="7"
  text="Select a value"
  type="select"
  style="display: inline-block; width: 150px">
