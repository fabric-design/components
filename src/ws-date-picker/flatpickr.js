...
function Flatpickr(element, config) {
  ...
  function bind() {
    if (self.config.wrap) {
      ['open', 'close', 'toggle', 'clear'].forEach(el => {
        ...
        for (let i = 0; i < toggles.length; i++)					{ toggles[i].addEventListener('click', self[el]); }
      });
    }
  ...
  function parseConfig() {
    ...
    for (let i = 0; i < hooks.length; i++)			{ self.config[hooks[i]] = arrayify(self.config[hooks[i]] || []).map(bindToInstance); }
  ...
  function positionCalendar(e) {
    ...
    const top = (window.pageYOffset + inputBounds.top) + (!showOnTop
  ...
  function selectDate(e) {
    ...
    if (e.which === 13 && self.config.enableTime)			{ setTimeout(() => self.hourElement.focus(), 451); }
  ...
  function setSelectedDate(inputDate, format) {
    if (Array.isArray(inputDate))			{ self.selectedDates = inputDate.map(d => self.parseDate(d, false, format)); } else if (inputDate instanceof Date || !isNaN(inputDate))			{ self.selectedDates = [self.parseDate(inputDate)]; } else if (inputDate && inputDate.substring) {
  ...
  function triggerEvent(event, data) {
    ...
    if (hooks) {
      for (let i = 0; i < hooks.length; i++)				{ hooks[i](self.selectedDates, self.input && self.input.value, self, data); }
    }
